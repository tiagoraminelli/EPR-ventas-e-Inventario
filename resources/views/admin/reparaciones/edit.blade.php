...
<body class="bg-gray-100 min-h-screen">
        ...
        <main class="flex-1 p-6">
            <div class="max-w-7xl mx-auto bg-white rounded-lg shadow p-6">
                ...
                <div class="flex justify-between items-center mb-6 border-b pb-3">
                    <div>
                        <h1 class="text-xl font-semibold text-gray-800">Editar Reparación #{{ $reparacion->id }}</h1>
                        <p class="text-xs text-gray-500 mt-0.5">Modifique la información requerida para actualizar la reparación</p>
                    </div>
                ...
                <form action="{{ route('reparaciones.update', $reparacion->id) }}" method="POST" id="reparacion-form" class="space-y-6">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="date" name="fecha_ingreso" id="fecha_ingreso"
                                    value="{{ old('fecha_ingreso', \Carbon\Carbon::parse($reparacion->fecha_ingreso)->format('Y-m-d')) }}"
                                    class="w-full border rounded px-3 py-2 text-sm"
                                    required>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="date" name="fecha_egreso" id="fecha_egreso"
                                    value="{{ old('fecha_egreso', $reparacion->fecha_egreso ? \Carbon\Carbon::parse($reparacion->fecha_egreso)->format('Y-m-d') : '') }}"
                                    class="w-full border rounded px-3 py-2 text-sm">
                            </div>
                            ...
                            <div>
                                ...
                                <select name="estado_reparacion" id="estado_reparacion"
                                    ...
                                    @foreach(['Pendiente', 'En proceso', 'Reparado', 'No reparable', 'Entregado'] as $estado)
                                        <option value="{{ $estado }}" {{ old('estado_reparacion', $reparacion->estado_reparacion) == $estado ? 'selected' : '' }}>
                                            {{ $estado }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="p-5">
                            <div>
                                ...
                                <textarea name="solucion_aplicada" id="solucion_aplicada" rows="3"
                                    class="w-full border rounded px-3 py-2 text-sm"
                                    placeholder="Describa la solución aplicada (si ya fue reparado)...">{{ old('solucion_aplicada', $reparacion->solucion_aplicada) }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white border rounded-lg overflow-hidden">
                        <div class="px-4 py-3 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
                            <div>
                                ...
                                <span id="productos-count" class="text-xs text-gray-500">{{ $reparacion->reparacionProductos->count() }} productos</span>
                            </div>
                        ...
                        <div class="overflow-x-auto">
                            <table id="productos-table" class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody id="productos-table-body" class="bg-white divide-y divide-gray-200">
                                    @foreach ($reparacion->reparacionProductos as $index => $detalle)
                    ...
                    <div class="bg-white border rounded-lg overflow-hidden">
                        <div class="px-4 py-3 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
                            <div>
                                ...
                                <span id="servicios-count" class="text-xs text-gray-500">{{ $reparacion->reparacionServicios->count() }} servicios</span>
                            </div>
                        ...
                        <div class="overflow-x-auto">
                            <table id="servicios-table" class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody id="servicios-table-body" class="bg-white divide-y divide-gray-200">
                                    @foreach ($reparacion->reparacionServicios as $index => $detalle)
                                    <tr class="detalle-servicio-row">
                                        <td class="px-3 py-2 whitespace-nowrap" data-label="Servicio">
                                            <input type="hidden" name="servicios[{{ $index }}][detalle_id]" value="{{ $detalle->id }}">
                    ...
                    <div class="bg-white border rounded-lg overflow-hidden">
                        ...
                        <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="bg-gray-50 p-3 rounded border">
                                ...
                                <p id="costo-total" class="text-lg font-bold text-gray-900">${{ number_format($reparacion->costo_total ?? 0, 2) }}</p>
                                <input type="hidden" name="costo_total" id="costo-total-input" value="{{ $reparacion->costo_total ?? 0 }}">
                            </div>
                            <div class="bg-blue-50 p-3 rounded border border-blue-200">
                                <span class="block text-xs text-blue-700 uppercase font-medium">IVA 21%</span>
                                <p id="importe-iva" class="text-lg font-bold text-blue-800">${{ number_format(($reparacion->costo_total ?? 0) * 0.21, 2) }}</p>
                                <input type="hidden" name="importe_iva" id="importe-iva-input" value="{{ ($reparacion->costo_total ?? 0) * 0.21 }}">
                            </div>
                        </div>
                    </div>
    ...
    <script>
        (function() {
            ...
            // Estado global
            const state = {
                productoIndex: {{ $reparacion->reparacionProductos->count() }},
                servicioIndex: {{ $reparacion->reparacionServicios->count() }},
            ...
            $(document).ready(function() {
                // Inicializar Select2
                $('.select2, .product-select, .service-select').select2({
                    width: '100%',
            ...
            function handleSubmit(e) {
                ...
                $btn.prop('disabled', true)
                    .html('<i class="fas fa-spinner fa-spin mr-2"></i> Actualizando...')
