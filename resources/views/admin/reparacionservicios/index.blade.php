...
<body class="bg-indigo-100 font-sans">
        ...
        <main class="flex-1 p-6">
            <div class="bg-white p-6 rounded-lg shadow-lg">
                <div class="flex flex-col md:flex-row justify-between items-center mb-6 border-b pb-4">
                    <h1 class="text-3xl font-bold text-gray-800 mb-4 md:mb-0">Servicios por Reparación</h1>
                ...
                <form action="{{ route('reparacionservicios.index') }}" method="GET" class="mb-6">
                    <div class="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4">
                        <input type="search" name="search" placeholder="Buscar por servicio..."
                        ...
                        <select name="servicios[]" multiple id="servicios-select"
                            class="px-4 py-2 border rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 w-full">
                ...
                <!-- Tabla de reparaciones y servicios -->
                            ...
                            @forelse($reparaciones as $reparacion)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="{{ route('reparacionservicios.edit', $reparacion->id) }}"
                                            class="text-indigo-600 hover:text-indigo-900">
                                            <i class="fas fa-edit"></i> Editar
                                        </a>
                                        <button onclick="openDeleteModal({{ $reparacion->id }}, this)"
                                            data-url="{{ route('reparacionservicios.destroy', $reparacion->id) }}"
    ...
    <script>
        ...
        function openDeleteModal(id, el) {
            const url = el.getAttribute('data-url'); // Tomamos la URL del data-attribute
