    ...
    <main class="flex-1 p-6">
        <div class="bg-white p-6 rounded-lg shadow-lg">
            <div class="flex flex-col md:flex-row justify-between items-center mb-6 border-b pb-4">
                <h1 class="text-3xl font-bold text-gray-800 mb-4 md:mb-0">Crear Servicios por Reparación</h1>
                        ...
                        <select name="reparacion_id" id="reparacion_id" required
                            ...
                            @foreach($reparaciones as $reparacion)
                                <option value="{{ $reparacion->id }}" {{ old('reparacion_id') == $reparacion->id ? 'selected' : '' }}>
                                    #{{ $reparacion->id }} - {{ optional($reparacion->cliente)->NombreCompleto ?? 'Sin cliente' }}
                                </option>
                            @endforeach
                        </select>
                    </div>
...
$(document).ready(function() {
        ...
        const serviceRow = `
            <div class="service-row border border-gray-200 p-4 rounded-lg mb-4 bg-white flex flex-col md:flex-row gap-4 items-center" data-index="${serviceIndex}">
                ...
                <div class="w-full md:w-1/5 flex justify-end md:justify-center mt-4 md:mt-0">
                    <button type="button" class="remove-service-btn px-4 py-2 bg-red-500 text-white font-bold rounded-md shadow-lg hover:bg-red-600 transition duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                        <i class="fas fa-trash-alt"></i>
                    </button>
                </div>
            </div>
        `;
        $('#servicios-container').append(serviceRow);
        $(`.service-select[name="servicios[${serviceIndex}][servicio_id]"]`).select2({ placeholder: "Seleccione un servicio" });
        serviceIndex++;
    }
    ...
    $('#servicios-container').on('change keyup', '.cantidad-input, .precio-input', function() {
        updateTotal();
    });
